...
export default function InputEmail({ name, title }: InputProps) {
    ...
    return <div className="form-control relative">
        <input type="email" className={`peer form-input border border-solid text-[13px] leading-[19px] p-md rounded-[4px] block w-full focus:border-primary ${isValid && `border-primary`} ${errors[name] && '!border-red-600'}`} id={name} required {...register(name, { ...errorMessage.required, ...errorMessage.email })} />
