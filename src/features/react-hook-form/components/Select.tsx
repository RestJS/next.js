...
export default function Select({ children, title, name }: SelectProps) {
    ...
    return <div className="form-control relative">
        <select className={`peer form-input border border-solid text-[13px] leading-[19px] p-md rounded-[4px] block w-full focus:border-primary valid:border-primary ${errors[name] && '!border-red-600'} appearance-none bg-white`} id={name} required {...register(name, errorMessage.required)}>
            <option value=""></option>
            {children ?? <>
        ...
        <BsChevronDown className="text-[16px] absolute right-[10px] top-[18px]" />
